...
struct SynE {
	static void deliver(lif& to, sim_info& info) {
		float const W = 0.4f * (16'000'000.0 / (info.network_size * info.network_size)); // siemens
		to.Gex += W;
	}
};
...
struct SynI {
	static void deliver(lif& to, sim_info& info) {
		float const W = 5.1f * (16'000'000.0 / (info.network_size * info.network_size)); // siemens
		to.Gin += W;
	}
};
