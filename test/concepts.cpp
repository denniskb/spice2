...
template <bool stateful, bool per_neuron_init, bool per_neuron_update, bool per_pop_init,
          bool per_pop_update, bool err>
struct meta_neuron : public std::conditional_t<stateful, stateful_neuron, stateless_neuron> {
	...
	bool update(float, auto) const requires(per_neuron_update && !stateful && !err) {
		return false;
	}
	...
	bool update(neuron_t&, float, auto) const requires(per_neuron_update&& stateful && !err) {
		return false;
	}
	void update(neuron_t&, float, auto) const requires(per_neuron_update&& stateful&& err) {}
	bool update(neuron_t&, float, auto) /* */ requires(per_neuron_update&& stateful&& err) {
		return false;
	}
...
TEST(Concepts, Neuron) {
	cartesian<5>([](auto stateful, auto per_neuron_init, auto per_neuron_update, auto per_pop_init,
		...
		static_assert(!Neuron<meta_neuron<stateful, per_neuron_init, per_neuron_update,
		                                  per_pop_init, per_pop_update, true>>);

		// Any neuron lacking an update() method isn't a Neuron
		static_assert(
		    !Neuron<meta_neuron<stateful, per_neuron_init, false, per_pop_init, false, false>>);
		...
		if constexpr (per_pop_update && any_of < stateful,
		              stateful && (per_neuron_init || per_pop_init), per_neuron_update >)
			static_assert(!Neuron<meta_neuron<stateful, per_neuron_init, per_neuron_update,
			                                  per_pop_init, per_pop_update, false>>);
...
template <bool stateful, bool deliver_to, bool deliver_from, bool plastic, bool per_syn_init,
          bool err>
struct meta_synapse : public std::conditional_t<stateful, stateful_synapse, stateless_synapse> {
	...
	void deliver(synapse_t const&, neuron_t const&,
	             neuron_t&) /* */ requires(stateful&& deliver_from&& err) {}
...
TEST(Concepts, Synapse) {
	cartesian<5>([](auto stateful, auto deliver_to, auto deliver_from, auto plastic,
	                auto per_syn_init) {
		// Any synapse with any errenous signature is not a Synapse
		static_assert(
		    !Synapse<meta_synapse<stateful, deliver_to, deliver_from, plastic, per_syn_init, true>,
		...
		static_assert(
		    PerSynapseInit<meta_synapse<true, deliver_to, deliver_from, plastic, true, false>>);
		static_assert(
		    !PerSynapseInit<meta_synapse<true, deliver_to, deliver_from, plastic, true, true>>);
		static_assert(
		    !PerSynapseInit<meta_synapse<true, deliver_to, deliver_from, plastic, false, false>>);
		...
		if constexpr (one_of<deliver_to, deliver_from> && (!plastic || stateful) &&
			...
			static_assert(
			    Synapse<
			        meta_synapse<stateful, deliver_to, deliver_from, plastic, per_syn_init, false>,
			        stateful_neuron, stateful_neuron>);
			...
			static_assert(
			    !Synapse<meta_synapse<stateful, false, true, plastic, per_syn_init, false>,
			             stateless_neuron, stateful_neuron>);
		}
	});
}